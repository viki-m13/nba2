...
def main():
    ...
    # Step 3: Generate tonight's picks
    print("\n--- Step 3: Generating tonight's picks ---")
    recommendations = generate_nightly_picks()

    if recommendations and recommendations.get('tonight_picks'):
        print(f"\nTonight's Picks ({len(recommendations['tonight_picks'])} recommendations):")
        for pick in recommendations['tonight_picks']:
            if pick.get('bet_type') == 'parlay':
                legs_str = ' + '.join(
                    f"{l['player']} O{l['line']} {l.get('stat', 'pts').upper()}"
                    for l in pick.get('legs', [])
                )
                print(f"  PARLAY: {legs_str} | Odds: {pick.get('parlay_odds')}")
            else:
                print(f"  {pick['bet_type'].upper()}: {pick['player']} O{pick['line']} "
                      f"{pick.get('stat', 'pts').upper()} | Odds: {pick['odds']} | "
                      f"Score: {pick.get('combined_score', 0):.3f}")
    else:
        print("\nNo picks generated for tonight (no games or no qualifying signals)")
